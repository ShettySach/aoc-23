...
fn cardsort<'a>(
    ...
    let strengths = HashMap::from([('T', 10), ('J', 1), ('Q', 12), ('K', 13), ('A', 14)]);
...
fn main() {
    ...
    lines.iter().for_each(|line| {
        ...
        if j_count > 0 {
            if let Some((&max_key, max_value)) = occurs.iter().max_by_key(|&(_, value)| value) {
                *occurs.get_mut(&max_key).unwrap() = max_value + j_count;
            } else {
